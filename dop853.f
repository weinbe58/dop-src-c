...
C --- THE TWELVE STAGES
      ...
      END IF
      DO 22 I=1,N 
C   22  Y1(I)=Y(I)+H*A21*K1(I)  
      CALL FCN(N,X+C2*H,Y1,K2,RPAR,IPAR)
      DO 23 I=1,N 
C   23  Y1(I)=Y(I)+H*(A31*K1(I)+A32*K2(I))  
      ALL FCN(N,X+C3*H,Y1,K3,RPAR,IPAR)
      DO 24 I=1,N 
C   24  Y1(I)=Y(I)+H*(A41*K1(I)+A43*K3(I)) 
      CALL FCN(N,X+C4*H,Y1,K4,RPAR,IPAR)
      DO 25 I=1,N 
C   25  Y1(I)=Y(I)+H*(A51*K1(I)+A53*K3(I)+A54*K4(I))
      CALL FCN(N,X+C5*H,Y1,K5,RPAR,IPAR)
      DO 26 I=1,N 
C   26  Y1(I)=Y(I)+H*(A61*K1(I)+A64*K4(I)+A65*K5(I))
      CALL FCN(N,X+C6*H,Y1,K6,RPAR,IPAR)
      DO 27 I=1,N 
C   27  Y1(I)=Y(I)+H*(A71*K1(I)+A74*K4(I)+A75*K5(I)+A76*K6(I))
      CALL FCN(N,X+C7*H,Y1,K7,RPAR,IPAR)
      DO 28 I=1,N 
C   28  Y1(I)=Y(I)+H*(A81*K1(I)+A84*K4(I)+A85*K5(I)+A86*K6(I)+A87*K7(I))  
      CALL FCN(N,X+C8*H,Y1,K8,RPAR,IPAR)
      DO 29 I=1,N 
C   29  Y1(I)=Y(I)+H*(A91*K1(I)+A94*K4(I)+A95*K5(I)+A96*K6(I)+A97*K7(I)
C      &   +A98*K8(I))
      CALL FCN(N,X+C9*H,Y1,K9,RPAR,IPAR)
      DO 30 I=1,N 
C   30  Y1(I)=Y(I)+H*(A101*K1(I)+A104*K4(I)+A105*K5(I)+A106*K6(I)
C      &   +A107*K7(I)+A108*K8(I)+A109*K9(I))
      CALL FCN(N,X+C10*H,Y1,K10,RPAR,IPAR)
      DO 31 I=1,N 
C   31  Y1(I)=Y(I)+H*(A111*K1(I)+A114*K4(I)+A115*K5(I)+A116*K6(I)
C      &   +A117*K7(I)+A118*K8(I)+A119*K9(I)+A1110*K10(I))
      CALL FCN(N,X+C11*H,Y1,K2,RPAR,IPAR)
      XPH=X+H
      DO 32 I=1,N 
C   32  Y1(I)=Y(I)+H*(A121*K1(I)+A124*K4(I)+A125*K5(I)+A126*K6(I)
C      &   +A127*K7(I)+A128*K8(I)+A129*K9(I)+A1210*K10(I)+A1211*K2(I))
...
C --- STEP IS ACCEPTED  
         ...
         NFCN=NFCN+1
C C ------- STIFFNESS DETECTION
C          IF (MOD(NACCPT,NSTIFF).EQ.0.OR.IASTI.GT.0) THEN
C             STNUM=0.D0
C             STDEN=0.D0
C             DO 64 I=1,N 
C                STNUM=STNUM+(K4(I)-K3(I))**2
C                STDEN=STDEN+(K5(I)-Y1(I))**2
C  64         CONTINUE  
C             IF (STDEN.GT.0.D0) HLAMB=ABS(H)*SQRT(STNUM/STDEN) 
C             IF (HLAMB.GT.6.1D0) THEN
C                NONSTI=0
C                IASTI=IASTI+1  
C                IF (IASTI.EQ.15) THEN
C                   IF (IPRINT.GT.0) WRITE (IPRINT,*) 
C      &               ' THE PROBLEM SEEMS TO BECOME STIFF AT X = ',X   
C                   IF (IPRINT.LE.0) GOTO 76
C                END IF
C             ELSE
C                NONSTI=NONSTI+1  
C                IF (NONSTI.EQ.6) IASTI=0
C             END IF
C          END IF 
C C ------- FINAL PREPARATION FOR DENSE OUTPUT
C          IF (IOUT.GE.2) THEN
C C ----    SAVE THE FIRST FUNCTION EVALUATIONS   
C             DO 62 J=1,NRD
C                I=ICOMP(J)
C                CONT(J)=Y(I)
C                YDIFF=K5(I)-Y(I)
...
C                BSPL=H*K1(I)-YDIFF
C                CONT(J+NRD*2)=BSPL
C                CONT(J+NRD*3)=YDIFF-H*K4(I)-BSPL
C                CONT(J+NRD*4)=D41*K1(I)+D46*K6(I)+D47*K7(I)+D48*K8(I)
C      &                  +D49*K9(I)+D410*K10(I)+D411*K2(I)+D412*K3(I)
C                CONT(J+NRD*5)=D51*K1(I)+D56*K6(I)+D57*K7(I)+D58*K8(I)
C      &                  +D59*K9(I)+D510*K10(I)+D511*K2(I)+D512*K3(I)
C                CONT(J+NRD*6)=D61*K1(I)+D66*K6(I)+D67*K7(I)+D68*K8(I)
C      &                  +D69*K9(I)+D610*K10(I)+D611*K2(I)+D612*K3(I)
C                CONT(J+NRD*7)=D71*K1(I)+D76*K6(I)+D77*K7(I)+D78*K8(I)
C      &                  +D79*K9(I)+D710*K10(I)+D711*K2(I)+D712*K3(I)
C    62       CONTINUE 
C C ---     THE NEXT THREE FUNCTION EVALUATIONS
C             DO 51 I=1,N 
C   51           Y1(I)=Y(I)+H*(A141*K1(I)+A147*K7(I)+A148*K8(I)
C      &            +A149*K9(I)+A1410*K10(I)+A1411*K2(I)+A1412*K3(I)
C      &            +A1413*K4(I))
C             CALL FCN(N,X+C14*H,Y1,K10,RPAR,IPAR)
C             DO 52 I=1,N 
C   52           Y1(I)=Y(I)+H*(A151*K1(I)+A156*K6(I)+A157*K7(I)
C      &            +A158*K8(I)+A1511*K2(I)+A1512*K3(I)+A1513*K4(I)
C      &            +A1514*K10(I))
C             CALL FCN(N,X+C15*H,Y1,K2,RPAR,IPAR)
C             DO 53 I=1,N 
C   53           Y1(I)=Y(I)+H*(A161*K1(I)+A166*K6(I)+A167*K7(I)
C      &            +A168*K8(I)+A169*K9(I)+A1613*K4(I)+A1614*K10(I)
C      &            +A1615*K2(I))
C             CALL FCN(N,X+C16*H,Y1,K3,RPAR,IPAR)
C             NFCN=NFCN+3 
C C ---     FINAL PREPARATION
C             DO 63 J=1,NRD
C                I=ICOMP(J)
C                CONT(J+NRD*4)=H*(CONT(J+NRD*4)+D413*K4(I)+D414*K10(I)
C      &            +D415*K2(I)+D416*K3(I))
C                CONT(J+NRD*5)=H*(CONT(J+NRD*5)+D513*K4(I)+D514*K10(I)
C      &            +D515*K2(I)+D516*K3(I))
C                CONT(J+NRD*6)=H*(CONT(J+NRD*6)+D613*K4(I)+D614*K10(I)
C      &            +D615*K2(I)+D616*K3(I))
C                CONT(J+NRD*7)=H*(CONT(J+NRD*7)+D713*K4(I)+D714*K10(I)
C      &            +D715*K2(I)+D716*K3(I))
C   63        CONTINUE
C             HOUT=H
C          END IF
         DO 67 I=1,N
  ...
  67     Y(I)=K5(I)
         ...
         X=XPH
C          IF (IOUT.GE.1) THEN
C             CALL SOLOUT(NACCPT+1,XOLD,X,Y,N,CONT,ICOMP,NRD,
C      &                  RPAR,IPAR,IRTRN)
C             IF (IRTRN.LT.0) GOTO 79
C          END IF 
C ------- NORMAL EXIT
         IF (LAST) THEN
            H=HNEW
